...
export default function StudentMirrorAsistenciasPage() {
    ...
    const studentRecords = useMemo(() => {
        if (!studentId) return [];
        return getAttendanceForStudent(studentId).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    }, [studentId, getAttendanceForStudent]);
    ...
    const attendanceStats = useMemo(() => {
        ...
        const effectiveTotal = total - justifiedCount;
        ...
        <div className="space-y-6">
             <Card>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                        <div className="p-4 bg-muted/50 rounded-lg"><p className="text-2xl font-bold">{attendanceStats.present}</p><p className="text-sm text-muted-foreground">Presente</p></div>
                        <div className="p-4 bg-muted/50 rounded-lg"><p className="text-2xl font-bold">{attendanceStats.absent}</p><p className="text-sm text-muted-foreground">Ausente</p></div>
                        <div className="p-4 bg-muted/50 rounded-lg"><p className="text-2xl font-bold">{attendanceStats.late}</p><p className="text-sm text-muted-foreground">Retardos</p></div>
                        <div className="p-4 bg-muted/50 rounded-lg"><p className="text-2xl font-bold">{attendanceStats.justified}</p><p className="text-sm text-muted-foreground">Justificado</p></div>
                    </div>
                </CardContent>
            </Card>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Registros de asistencia tomados por los docentes.</CardDescription>
                </CardHeader>
                <CardContent>
                    <Table>
                        <TableHeader>
